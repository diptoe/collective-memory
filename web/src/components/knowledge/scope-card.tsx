...
const SCOPE_COLORS: Record<string, { bg: string; border: string; accent: string }> = {
  domain: { bg: 'bg-blue-50', border: 'border-blue-200', accent: 'bg-blue-500' },
  team: { bg: 'bg-green-50', border: 'border-green-200', accent: 'bg-green-500' },
  user: { bg: 'bg-purple-50', border: 'border-purple-200', accent: 'bg-purple-500' },
};
...
export function ScopeCard({ scope, totalEntities }: ScopeCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        "block rounded-xl border p-4 transition-all hover:shadow-md hover:-translate-y-0.5",
        colors.bg,
        colors.border
      )}
    >
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span className="text-xs text-cm-coffee bg-white/80 px-2 py-0.5 rounded-full">
          {percentage}%
        </span>
      </div>
      ...
      {topTypes.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {topTypes.map(([type, count]) => (
            <span
              key={type}
              className="text-xs bg-white/80 text-cm-coffee px-2 py-0.5 rounded"
            >
              {type}: {count}
            </span>
          ))}
